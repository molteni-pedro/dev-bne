...
global $wpdb;
$table = $wpdb->prefix . 'bne_modalidad';
if (!empty($_POST)) {
	
	$data = array(
		'MOtitulo' => $_POST['MOtitulo'],
	);
	...
	Redirect("admin.php?page=modalidad");
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE MOid IN ($ids)");
	}
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Modalidad', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=modalidad&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE MOid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="MOtitulo" value="<?php echo $sValor;?>" id="MOtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY MOid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['MOid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['MOtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=modalidad&action=edit&id=<?php echo  $rowDatos['MOid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=modalidad&action=delete&id=<?php echo  $rowDatos['MOid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=modalidad";
			}
