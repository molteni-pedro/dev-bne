...
global $wpdb;
$table = $wpdb->prefix . 'bne_nivelagregacion';
if (!empty($_POST)) {
	
	$data = array(
		'NAtitulo' => $_POST['NAtitulo'],
	);
	...
	Redirect("admin.php?page=nivel-agregacion");
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE NAid IN ($ids)");
	}
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Nivel Agregación', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=nivel-agregacion&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE NAid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="NAtitulo" value="<?php echo $sValor;?>" id="NAtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY NAid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['NAid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['NAtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=nivel-agregacion&action=edit&id=<?php echo  $rowDatos['NAid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=nivel-agregacion&action=delete&id=<?php echo  $rowDatos['NAid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=nivel-agregacion";
			}
