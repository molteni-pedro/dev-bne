...
global $wpdb;
$table = $wpdb->prefix . 'bne_bloque';
if (!empty($_POST)) {
	
	$data = array(
		'BLtitulo' => $_POST['BLtitulo'],
	);
	...
	Redirect("admin.php?page=bloques");
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE BLid IN ($ids)");
	}
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Bloques', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'recursoslisttable');?>" class="fa fa-plus ico-boton30"  href="?page=bloques&action=edit&id=0"><span><?php echo __('Nuevo', 'recursoslisttable');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE BLid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="BLtitulo" value="<?php echo $sValor;?>" id="BLtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY BLid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['BLid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['BLtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=bloques&action=edit&id=<?php echo  $rowDatos['BLid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=bloques&action=delete&id=<?php echo  $rowDatos['BLid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=bloques";
			}
