...
global $wpdb;
$table = $wpdb->prefix . 'bne_restriccion';
if (!empty($_POST)) {
	
	$data = array(
		'RStitulo' => $_POST['RStitulo'],
	);
	...
	Redirect("admin.php?page=restricciones");
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE RSid IN ($ids)");
	}
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Restricciones', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=restricciones&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE RSid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="RStitulo" value="<?php echo $sValor;?>" id="RStitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY RSid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['RSid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['RStitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=restricciones&action=edit&id=<?php echo  $rowDatos['RSid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=restricciones&action=delete&id=<?php echo  $rowDatos['RSid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=restricciones";
			}
