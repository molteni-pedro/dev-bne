...
global $wpdb;
$table = $wpdb->prefix . 'bne_metadatoscatalogo';
if (!empty($_POST)) {
	
	$data = array(
		'MCtitulo' => $_POST['MCtitulo'],
	);
	...
	Redirect("admin.php?page=esquema-meta-catalogo");
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE MCid IN ($ids)");
	}
			...
			<div class="col-md-12">
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=esquema-meta-catalogo&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE MCid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="MCtitulo" value="<?php echo $sValor;?>" id="MCtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY MCid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					<thead>
						<tr>
							<th class="column-id">
								<?php echo __('Id', 'traduccionrecursos', 'traduccionrecursos');?>
							</th>
							<th>
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['MCid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['MCtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=esquema-meta-catalogo&action=edit&id=<?php echo  $rowDatos['MCid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=esquema-meta-catalogo&action=delete&id=<?php echo  $rowDatos['MCid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=esquema-meta-catalogo";
			}
