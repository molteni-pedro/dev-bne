...
if (!empty($_POST)) {
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Etapa', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=etapa&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE ETid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="ETtitulo" value="<?php echo $sValor;?>" id="ETtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY ETid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['ETid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['ETtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=etapa&action=edit&id=<?php echo  $rowDatos['ETid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=etapa&action=delete&id=<?php echo  $rowDatos['ETid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=etapa";
			}
