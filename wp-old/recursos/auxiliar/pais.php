...
global $wpdb;
$table = $wpdb->prefix . 'bne_paises';
if (!empty($_POST)) {
	
	$data = array(
		'PAtitulo' => $_POST['PAtitulo'],
	);
	...
	if (!empty($ids)) {
		$wpdb->query("DELETE FROM $table WHERE PAid IN ($ids)");
	}
				...
				<div class="widget-box transparent invoice-box">
					<div class="widget-header">
						<h3 class="widget-title lighter">
							<span><?php echo __('Paises', 'traduccionrecursos');?></span>
						</h3>
						...
						<?php }else{ ?>
						<div class="widget-toolbar">
							<a id="btnAceptar" tabindex="6" title="<?php echo __('Nuevo', 'traduccionrecursos');?>" class="fa fa-plus ico-boton30"  href="?page=pais&action=edit&id=0"><span><?php echo __('Nuevo', 'traduccionrecursos');?></span></a>
						</div>
						<?php } ?>
					</div>
				</div>
	...
	switch ($_GET['action']) {
		case 'edit':
			...
			if($id > 0){
				$aValores = array($id);
				$qry = $wpdb->prepare("SELECT * FROM ".$table." WHERE PAid = %d", $aValores);
						...
						<fieldset>
							...
							<div class="row">
								<div class="col-xs-12 control-label text-left required">
									...
									<input name="PAtitulo" value="<?php echo $sValor;?>" id="PAtitulo" class="form-control" type="text">
								</div>
							</div>
						</fieldset>
					</div>
		...
		case 'list':
		default:
			$aTabla = $wpdb->get_results("SELECT * FROM $table ORDER BY PAid",	ARRAY_A);
				...
				<table class="wp-list-table widefat striped">
					...
					<tbody>
						...
						foreach ($aTabla as $rowDatos): array_map('htmlentities', $rowDatos); 
							...
							<tr>
								<th class="column-id">
									<?php echo  $rowDatos['PAid']; ?>
								</th>
								<td><div class="contenido"><?php echo  $rowDatos['PAtitulo']; ?></div>
									<div class="row-actions">
										<span class="edit">
											<a href="?page=pais&action=edit&id=<?php echo  $rowDatos['PAid']; ?>">
												Editar
											</a>
											 | 
										</span>
											<span class="delete">
												<a href="?page=pais&action=delete&id=<?php echo  $rowDatos['PAid']; ?>">
												Eliminar
											</a>
										</span>
									</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			function Salir(){
				location.href = "admin.php?page=pais";
			}
